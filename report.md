...
| 阶段 | 时长 | A100 费用 | A800 费用（推荐） |
|------|------|----------|------------------|
| 真实数据生成（8 电路 × 8 种子） | ~1.5h | ~¥45 | ~¥35 |
| NeuralVSR 训练（real_traj_v2） | 0.5h | ~¥15 | ~¥12 |
| ISPD2005 全量评估 | ~2h | ~¥60 | ~¥48 |
| Buffer | ~1h | ~¥30 | ~¥24 |
| **总计** | **~5h** | **~¥150** | **~¥120** |

**A800 80GB 是同样可用的选择**（国内版 A100，显存/算力/带宽完全一样，只差 NVLink，对单卡无影响）。AutoDL 上通常便宜 20-25%，优先选 A800。
...
批准在 AutoDL 上租 **A100 或 A800 80GB**（优先 A800，一样可用还便宜）~5 小时（~¥120-150），执行 Day-7 计划：
