...
\paragraph{Diffusion for placement.}  ChipDiffusion~\cite{chipdiffusion}
introduced a graph-conditioned diffusion model for macro placement.  Concurrent
work has explored autoregressive~\cite{miraichip} and flow-matching
\cite{chipflow} generators; all share the same defect of ignoring hard
constraints.
...
\caption{Per-circuit Δviolation and ΔHPWL means (\,$\pm$\,std), 4 seeds each.
...
generalizes to other diffusion-based placement backbones (e.g.\ flow-matching
\cite{chipflow}) is open.
...
\caption{VSR pipeline: verifier $\rightarrow$ selector $\rightarrow$
post-processing or intra-sampling repair operator.  No backbone retraining;
no differentiable surrogate.}
\label{fig:framework}
\end{figure*}
...
\caption{Before/after placement visualization on adaptec1 and adaptec3
(per-macro violation severity, log scale; yellow = low, dark red = high).
VSR-post at $\lambda=2$ reduces severity while preserving spatial structure.}
...
\caption{bigblue3 (1298 macros, seed=42).  The raw diffusion draft (left)
piles macros at a corner.  Post-processing VSR-post (middle) spreads them
out to achieve $-79\%$ violations but grows wirelength by $18\%$.
Intra-sampling VSR-intra (right) retains a compact layout that respects
the diffusion backbone's implicit wirelength prior, achieving the strongest
HPWL reduction.}
