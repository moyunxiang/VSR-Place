...
The legalizer optimizes a fixed differentiable surrogate on the \emph{full}
placement, with all macros free.  Our operator exploits two pieces of
structure: (i) a \emph{selective mask}---only offending macros
move, preserving the global arrangement the backbone learned; (ii) a
\emph{pairwise force field}---repulsion between two specifically overlapping
macros is a far better correction signal than the gradient of an aggregated
softmax.  The lambda sweep (\S\ref{sec:lambda_sweep}) shows the result
empirically: at $\lambda{=}4$ a $50\times$ shorter run strictly
Pareto-dominates the draft on $89\%$ of trials.

\paragraph{Why do learned repair operators not help here?}  Our exploration
of GNN-based repair (six training regimes, supplement) consistently failed to
match the hand-crafted force integrator.  ISPD2005 has only 8 circuits;
a learned operator must discover the same inductive bias (pairwise overlap,
edge-local wirelength, selective preservation) that our operator encodes by
construction.  At this data scale the bias dominates; substantially larger
circuit corpora would be needed for a learned operator to compete.
