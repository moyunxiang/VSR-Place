...
\caption{Bigblue3 (1298 macros) qualitative comparison.  Color = per-macro
violation severity (log scale, dark red = high).  Raw draft (left) piles
macros at one corner.  VSR-post (middle) spreads them out: violations drop
sharply but HPWL grows.  Intra-sampling (right) keeps a compact layout by
re-invoking the diffusion backbone's wirelength prior.  Quantitative numbers
for all 4 seeds in Table~\ref{tab:main}.}
