...
selectors match the full operator on these worst-case drafts (every macro
violating); the difference is exposed in the 2-round protocol of
\S\ref{sec:lambda_sweep}.  The boundary-only signal turns out to be
incidental---repulsive and attractive forces handle in-bounds movement
without it---a minor finding we leave to the supplement.}
...
severity-weighted alternative; the selector is essential only when offenders
are a strict subset (later iterative rounds, not first-pass repair).
