...
\paragraph{Robustness checks.} Two reviewer-prompted controls in the
supplement: (i) full-design HPWL with cells held at industrial
\texttt{.pl} (\S\ref{sec:supp-full-hpwl}), median
$\Delta h{=}{+}10.7\%$ for VSR-post and $-1.6\%$ for VSR-intra-soft
--- the macros-only trend generalises, with adaptec3 the lone
exception ($+73\%$ full-HPWL, due to dense cell-macro coupling that the
macro-only operator does not see); (ii) per-circuit tuning of cg
($w\in\{0.5,1,2,4,8\}$) and RePaint ($t_\text{start}\in\{0.1,...,0.7\}$),
\S\ref{sec:supp-tuned-baselines}: at the best hp per circuit, cg-tuned
medians $\Delta v{=}{-}7\%/\Delta h{=}{+}14\%$ and RePaint-tuned
$\Delta v{=}{+}109\%/\Delta h{=}{-}53\%$ (RePaint's HPWL ``win'' comes
from collapsing macros into overlap), versus VSR-post
$\Delta v{=}{-}55\%/\Delta h{=}{-}11\%$.  VSR-post strictly dominates
both tuned baselines on $\Delta v$ for every circuit.
...
A timestep sweep
$t_{\text{start}} \in \{0.1, 0.2, 0.3, 0.5, 0.7\}$ on all 6 circuits / 3
seeds (Fig.~\ref{fig:supp-timestep}, supplement) shows: small effect on
small circuits, modest ($-5\%$ to $-13\%$ HPWL) on adaptec3/4, but
dramatic on the largest circuit (bigblue3, 1298 macros):
\textbf{$-34.5\%$ HPWL at $t_{\text{start}}=0.5$} with violations
essentially unchanged.  The monotonic trend on bigblue3 is consistent
with the hypothesis that more re-noising more strongly re-invokes the
backbone prior.
